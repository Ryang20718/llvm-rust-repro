...
def rustc_compile_action(
    ...
    # https://github.com/bazelbuild/rules_rust/issues/2697 dynamic libraries are
    # now uses LibraryToLink.dynamic_library which may be None
    dynamic_libraries_filtered = [lib for lib in dynamic_libraries if lib != None]
    runfiles = ctx.runfiles(
        files = getattr(ctx.files, "data", []) +
                ([] if experimental_use_coverage_metadata_files else coverage_runfiles) +
                dynamic_libraries_filtered,
        collect_data = True,
    )
